...
unsigned int new_shader_program(const string program_name) {
  ...
  // 3 characters .vs
  char* vertex_shader_path = (char*) malloc((len) * sizeof(char));

  strcpy(vertex_shader_path, shader_path);
  strcat(vertex_shader_path, program_name.c_str());
  strcat(vertex_shader_path, ".vert");

  char* fragment_shader_path = (char*) malloc((len) * sizeof(char));
  strcpy(fragment_shader_path, shader_path);
  strcat(fragment_shader_path, program_name.c_str());
